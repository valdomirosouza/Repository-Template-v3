...
variable "service_account_name" {
  description = "Kubernetes ServiceAccount name for event-worker"
  type        = string
  default     = "event-worker"
}

variable "msk_cluster_arn" {
  description = "MSK (Managed Streaming for Kafka) cluster ARN — grants the worker read/write access"
  type        = string
  default     = ""
}

variable "dlq_sqs_arn" {
  description = "SQS ARN for the dead-letter queue — grants the worker send access"
  type        = string
  default     = ""
}
