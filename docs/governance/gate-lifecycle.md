...
| IaC scan — Checkov (ADR-0029 §1)                      | `ci.yml` → _IaC Security Scan (Checkov)_                | **report**   | in progress (started; awaiting first runs)             | Scans `infrastructure/` (TF + Helm + K8s), SARIF to Security tab  |
